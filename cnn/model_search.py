...
class Network(nn.Module):
  ...
  def generate_weight_random_alphas(self, acc_mat_normal, acc_mat_reduce):
    self._initialize_alphas()
    ...
    for i in range(self._steps):
      ...
      random.shuffle(indices)
      for e in indices[:2]:
        selected_op = random.choices([i for i in range(7)], acc_mat_normal[e])
        self.alphas_normal[e][selected_op] = 1.0

      random.shuffle(indices)
      for e in indices[:2]:
        selected_op = random.choices([i for i in range(7)], acc_mat_reduce[e])
        self.alphas_reduce[e][selected_op] = 1.0
