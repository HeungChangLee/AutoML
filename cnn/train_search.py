...
CIFAR_CLASSES = 10
model_path = 'search-RL-s2/weights.pt'
node_num = 4
...
def main(seed):
  ...
  top_arch_df = pd.DataFrame(columns = ['Normal', 'Reduce', 'Val_acc'])
  for epoch in range(args.epochs):
    ...
    valid_acc, top_arch_df = train_arch(train_queue, valid_queue, model, architect, top_arch_df)
...
def train_arch(train_queue, valid_queue, model, architect, top_arch_df):    
  top1 = utils.AvgrageMeter()  
  for step, (input, target) in enumerate(valid_queue):
    ...
    #Apply gradient for every arch_opt_steps
    if (step + 1) % arch_opt_step == 0:
      is_opt_step = True
    else:
      is_opt_step = False
    architect.step(input, target, is_opt_step) 
...
def best_arch_eval(valid_queue, model, top_arch_df):    
    model.eval()    
    result_df = pd.DataFrame(columns = ['Normal', 'Reduce', 'Full_val_acc', 'Single_val_acc'])
    for i in range(top_arch_num):
      top1 = utils.AvgrageMeter()
      model.sampled_weight_normal = Variable(torch.Tensor(top_arch_df['Normal'].loc[i]), requires_grad=False).cuda()
      model.sampled_weight_reduce = Variable(torch.Tensor(top_arch_df['Reduce'].loc[i]), requires_grad=False).cuda()
      for step, (input, target) in enumerate(valid_queue):        
        n = input.size(0)
      ...
      gene_normal = w_parse(model.sampled_weight_normal.data.cpu().numpy())
      gene_reduce = w_parse(model.sampled_weight_reduce.data.cpu().numpy())
      temp_df = pd.DataFrame([[gene_normal, gene_reduce, top1.avg, top_arch_df['Val_acc'].loc[i]]], columns = ['Normal', 'Reduce', 'Full_val_acc', 'Single_val_acc'])
      result_df = result_df.append(temp_df, ignore_index=True)
    result_df = result_df.sort_values(by='Full_val_acc', ascending=False)
    result_df.to_csv('search_result.csv')
